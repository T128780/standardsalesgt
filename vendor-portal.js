...
function switchVendorTab(tab) {
  ...
  document.querySelectorAll("[data-vendor-view]").forEach(view => {
    view.hidden = view.datasetVendorView !== tab;
  });
...
async function submitVendorProfileChange(form) {
  ...
  if (!changes.departamento || !changes.origenes || !changes.marcas || !changes.lineas || !changes.categorias) {
    toast("Completa departamento, orígenes, marcas, líneas y categorías.", "error");
    return;
  }
  const session = requireLocalVendorSession();
